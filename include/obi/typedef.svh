...
// Each `OBI_TYPEDEF_*` macro is defined in terms of an `OBI_DECL_*` macro expanding to the bare
// (anonymous) struct. The `OBI_DECL_*` form can be used wherever a type expression is expected,
// most notably in a parameter port list:
//
// #(
//   localparam type a_optional_t = `OBI_DECL_ALL_A_OPTIONAL(1, 1, 1, 1),
//   localparam type a_chan_t     = `OBI_DECL_A_CHAN_T(32, 32, 4, a_optional_t)
// )
...
`define OBI_TYPEDEF_A_CHAN_T(a_chan_t, ADDR_WIDTH, DATA_WIDTH, ID_WIDTH, a_optional_t) \
  typedef `OBI_DECL_A_CHAN_T(ADDR_WIDTH, DATA_WIDTH, ID_WIDTH, a_optional_t) a_chan_t;

`define OBI_DECL_TYPE_A_CHAN_T(addr_t, data_t, strb_t, id_t, a_optional_t) \
  struct packed {                                                          \
    addr_t       addr;                                                     \
    logic        we;                                                       \
    strb_t       be;                                                       \
...
`define OBI_TYPEDEF_TYPE_A_CHAN_T(a_chan_t, addr_t, data_t, strb_t, id_t, a_optional_t) \
  typedef `OBI_DECL_TYPE_A_CHAN_T(addr_t, data_t, strb_t, id_t, a_optional_t) a_chan_t;

`define OBI_DECL_MINIMAL_A_OPTIONAL logic
`define OBI_TYPEDEF_MINIMAL_A_OPTIONAL(a_optional_t) \
  typedef `OBI_DECL_MINIMAL_A_OPTIONAL a_optional_t;

`define OBI_DECL_ATOP_A_OPTIONAL  \
  struct packed {                 \
    obi_pkg::atop_t atop;         \
  }
`define OBI_TYPEDEF_ATOP_A_OPTIONAL(a_optional_t) \
  typedef `OBI_DECL_ATOP_A_OPTIONAL a_optional_t;
...
`define OBI_TYPEDEF_ALL_A_OPTIONAL(a_optional_t, AUSER_WIDTH, WUSER_WIDTH, MID_WIDTH, ACHK_WIDTH) \
  typedef `OBI_DECL_ALL_A_OPTIONAL(AUSER_WIDTH, WUSER_WIDTH, MID_WIDTH, ACHK_WIDTH) a_optional_t;
...
`define OBI_TYPEDEF_R_CHAN_T(r_chan_t, RDATA_WIDTH, ID_WIDTH, r_optional_t) \
  typedef `OBI_DECL_R_CHAN_T(RDATA_WIDTH, ID_WIDTH, r_optional_t) r_chan_t;

`define OBI_DECL_TYPE_R_CHAN_T(data_t, id_t, r_optional_t) \
  struct packed {                                          \
    data_t       rdata;                                    \
    id_t         rid;                                      \
    logic        err;                                      \
...
`define OBI_TYPEDEF_TYPE_R_CHAN_T(r_chan_t, data_t, id_t, r_optional_t) \
  typedef `OBI_DECL_TYPE_R_CHAN_T(data_t, id_t, r_optional_t) r_chan_t;

`define OBI_DECL_MINIMAL_R_OPTIONAL logic
`define OBI_TYPEDEF_MINIMAL_R_OPTIONAL(r_optional_t) \
  typedef `OBI_DECL_MINIMAL_R_OPTIONAL r_optional_t;
...
`define OBI_TYPEDEF_ALL_R_OPTIONAL(r_optional_t, RUSER_WIDTH, RCHK_WIDTH) \
  typedef `OBI_DECL_ALL_R_OPTIONAL(RUSER_WIDTH, RCHK_WIDTH) r_optional_t;

`define OBI_DECL_DEFAULT_REQ_T(a_chan_t) \
  struct packed {                        \
    a_chan_t a;                          \
    logic    req;                        \
  }
`define OBI_TYPEDEF_DEFAULT_REQ_T(req_t, a_chan_t) \
  typedef `OBI_DECL_DEFAULT_REQ_T(a_chan_t) req_t;

`define OBI_DECL_REQ_T(a_chan_t) \
  struct packed {                \
    a_chan_t a;                  \
    logic    req;                \
    logic    rready;             \
...
`define OBI_TYPEDEF_REQ_T(req_t, a_chan_t) \
  typedef `OBI_DECL_REQ_T(a_chan_t) req_t;

`define OBI_DECL_RSP_T(r_chan_t) \
  struct packed {                \
    r_chan_t r;                  \
    logic    gnt;                \
    logic    rvalid;             \
...
`define OBI_TYPEDEF_RSP_T(rsp_t, r_chan_t) \
  typedef `OBI_DECL_RSP_T(r_chan_t) rsp_t;
...
`define OBI_TYPEDEF_INTEGRITY_REQ_T(req_t, a_chan_t) \
  typedef `OBI_DECL_INTEGRITY_REQ_T(a_chan_t) req_t;
...
`define OBI_TYPEDEF_INTEGRITY_RSP_T(rsp_t, r_chan_t) \
  typedef `OBI_DECL_INTEGRITY_RSP_T(r_chan_t) rsp_t;
